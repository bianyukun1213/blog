...
class SmI18n {
    ...
    initPopContentHtml(minimumSupportedBrowserVersions, referrerKey, postProcessor) {
        ...
        if (this.isEn()) {
            ...
            <p>Most resources on this site are hosted outside Chinese mainland and may not load properly under the network environment in Chinese mainland. If you visit this site in Chinese mainland, it is recommended to use a proxy. Some of the features used on this site require Chrome/Chromium ${minimumSupportedBrowserVersions.chrome}, Safari ${minimumSupportedBrowserVersions.safari}, Firefox ${minimumSupportedBrowserVersions.firefox} or above. Some content such as post summaries and image captions may have been generated by AI and are for reference only. There are few accessibility functions implemented on this site, so if you have trouble browsing this site, consider using <a href="/${this.#lang}/atom.xml" target="_blank">the RSS feed</a>.</p>
            ...
            <p>本站的大部分资源托管在国外，在中国大陆的网络环境下可能无法正常加载。如果您在中国大陆访问本站，推荐使用代理。本站使用的部分特性需要 Chrome/Chromium ${minimumSupportedBrowserVersions.chrome}、Safari ${minimumSupportedBrowserVersions.safari}、Firefox ${minimumSupportedBrowserVersions.firefox} 及以上版本的支持。部分内容如文章总结及图片标注可能由 AI 生成，仅供参考。本站缺失无障碍实现，如果您在浏览时遇到困难，可尝试 <a href="/${this.#lang}/atom.xml" target="_blank">RSS 订阅</a>。</p>
